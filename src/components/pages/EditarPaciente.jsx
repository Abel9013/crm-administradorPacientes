...
export async function loader({params}){
    const paciente = await obtenerPaciente(params.pacienteId)
    if(Object.values(paciente).length===0){
      throw new Response('',{
        status:404,
        statusText: 'Paciente no encontrado'
      })
    }
    return paciente
}
export async function action({request, params}){
    ...
    await actualizarPaciente(params.pacienteId,datos)
...
const EditarPaciente = () => {
  ...
  return (
    <>
      ...
      <p className="mt-3">A continuacion podras modificar los datos del paciente</p>
